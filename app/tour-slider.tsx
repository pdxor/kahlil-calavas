...
export default function TourSlider() {
  ...
  useEffect(() => {
    ...
    return () => { api.off('select', update); api.off('reInit', update); api.off('slidesInView', update); };
  }, [api]);
      ...
      <CarouselContent className="world-track">
        {tours.map((tour, index) => (
          <CarouselItem className="world-slide" key={tour.slug} aria-label={`${index + 1} of ${tours.length}: ${tour.name}`}>
            <a className="world-image" href={tour.url} target="_blank" rel="noopener noreferrer" aria-label={`Explore ${tour.name} — opens the digital twin in a new tab`} tabIndex={visible.includes(index) ? 0 : -1}>
            ...
            <div className="world-caption"><div><p className="eyebrow">{tour.location}</p><h3><a href={tour.url} target="_blank" rel="noopener noreferrer" tabIndex={visible.includes(index) ? 0 : -1}>{tour.name}</a></h3></div><p className="world-note">{tour.note}</p></div>
          </CarouselItem>
        ))}
      </CarouselContent>
